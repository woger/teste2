...
namespace Settings.Configuracoes
{
    ...
    {
        public static string DIRETORIO_INSTALACAO = System.Windows.Forms.Application.StartupPath;

        public static string DIRETORIO_INSTALACAO_PALESTRAS = System.Windows.Forms.Application.StartupPath + @"\PALESTRAS\";

        public static string DIRETORIO_INSTALACAO_PALESTRAS_SALA(string IP)
        {
            return @"\\" + IP + @"\PALESTRAS\";
        }
        ...
        //{
        //    string[] files = Directory.GetFiles(ArquivoBD.DIRETORIO_INSTALACAO);
        //    for (int i = 0; i < files.Length; i++)
        //    {
        //        string extensao = Path.GetExtension(files[i]);
        //        if(extensao == ".DBF")
        //        {
        //            FileSecurity fSecurity = File.GetAccessControl(files[i]);
        ...
        //            File.SetAccessControl(files[i], fSecurity);
        ...
        public static void CriarArquivosBD()
        {
            ...
            using (OleDbConnection oConn = new OleDbConnection(@"Provider=Microsoft.Jet.OLEDB.4.0; Data Source=" + ArquivoBD.DIRETORIO_INSTALACAO + ";Extended Properties=dBase III"))
            {
                ...
                try
                {
                    using (OleDbCommand cmd = new OleDbCommand("  CREATE TABLE SALAS.DBF ([ID] NUMERIC(2,0), [NOME] CHAR(30), [IP] CHAR(15));"))//this works and creates an empty .dbf file
                    {
                        cmd.Connection = oConn;
                        cmd.ExecuteNonQuery();
                    }
                ...
                //Seto as permissões nos arquivos de BD - FullControl
                string[] files = Directory.GetFiles(ArquivoBD.DIRETORIO_INSTALACAO);
                for (int i = 0; i < files.Length; i++)
                {
                    string extensao = Path.GetExtension(files[i]);
        ...
        public static void CriarArquivosCronometroBD()
        {
            //string dbfDirectory = @"C:\the_path_to_my_dbf_file_or_files";
            ...
            using (OleDbConnection oConn = new OleDbConnection(@"Provider=Microsoft.Jet.OLEDB.4.0; Data Source=" + ArquivoBD.DIRETORIO_INSTALACAO + ";Extended Properties=dBase III"))
            {
                ...
                try
                {
                    using (OleDbCommand cmd = new OleDbCommand("  CREATE TABLE HORARIO.DBF ([ID] NUMERIC (18,0), [NOME] CHAR(50), TEMPO CHAR(150));"))//this works and creates an empty .dbf file
                    {
                        cmd.Connection = oConn;
                        cmd.ExecuteNonQuery();
                    }
